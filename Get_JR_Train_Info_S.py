...
class train_status(commands.Cog):
  ...
  @discord.app_commands.command(name = "s", description = "札幌～小樽運行情報")
  ...
  @discord.app_commands.choices(text2=[
    ...
    discord.app_commands.Choice(name="o", value="o"),
    discord.app_commands.Choice(name="oa", value="oa")
  ])

  async def s(self, ctx: discord.Interaction, text: int, text2: str, text3: int=5, x: bool = False, y: bool = True):
    ...
    for i in range(15):
      if (int(codelist[i]["駅番号"]) == int(text)):
        gyou = text - 1
...
async def setup(bot):
  await bot.add_cog(train_status(bot))