...
class JR_Train_Info_H(commands.Cog):
  ...
  @discord.app_commands.command(name = "h", description = "千歳線（札幌～苫小牧）列車運行情報")
  ...
  async def a(self, ctx: discord.Interaction, text: int, text2: str, text3: int=5, x: bool = False, y: bool = True):
    ...
    for i in range(18):
      if (int(codelist[i+15]["駅番号"]) == int(text)):
        gyou = text + 14
...
async def setup(bot):
  await bot.add_cog(JR_Train_Info_H(bot))