...
class JR_Train_Info_A(commands.Cog):
  ...
  @discord.app_commands.command(name = "a", description = "札幌～岩見沢　列車運行情報")
  ...
  @discord.app_commands.choices(text2=[
    ...
    discord.app_commands.Choice(name="ia", value="ia")
  ])

  async def a(self, ctx: discord.Interaction, text: int, text2: str, text3: int=5, x: bool = False, y: bool = True):
    ...
    for i in range(13):
      if (int(codelist[i+33]["駅番号"]) == int(text)):
        gyou = text +32
...
async def setup(bot):
  await bot.add_cog(JR_Train_Info_A(bot))
